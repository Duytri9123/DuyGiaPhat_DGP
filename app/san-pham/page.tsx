...
function ProductsContent() {
  ...
  const productsInCategory = useMemo(() => {
    if (pathname) {
      return mockProducts.filter((p) => p.category === pathname);
    }
    return mockProducts;
  }, [pathname]);
  ...
  const currentCategory = categories.find((c) => c.slug === pathname);
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] pt-20 md:pt-32">
      <SEO
        ...
        url={pathname ? `https://duygiaphat.vn/san-pham?category=${pathname}` : 'https://duygiaphat.vn/san-pham'}
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <aside className="hidden lg:block w-72 flex-shrink-0 sticky top-28 h-fit bg-white rounded-xl p-6 shadow-sm border border-slate-200/60">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-col gap-1 text-sm">
              <label className="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-50 cursor-pointer group">
                ...
                />
                <span
                  className={`font-medium ${!pathname ? 'text-slate-900' : 'text-slate-700 group-hover:text-slate-900'}`}
                >
                  Tất cả sản phẩm
                </span>
              </label>
              {categories.map((cat) => (
                ...
                >
                  <input
                    type="radio"
                    name="category-sidebar"
                    checked={pathname === cat.slug}
                    onChange={() => router.push(`/danh-muc/${cat.slug}`)}
                    className="h-4 w-4 text-amber-500 border-slate-300 focus:ring-amber-500"
                  />
                  <span
                    className={`flex-1 text-sm ${
                      pathname === cat.slug ? 'font-semibold text-slate-900' : 'text-slate-700 group-hover:text-slate-900'
                    }`}
                  >
                    {cat.name}
                  </span>
          ...
          {(pathname || searchQuery || filterInStock !== null) && (
            <button
              onClick={() => {
                  ...
                  <ul className="space-y-2">
                    <li>
                      <label className="flex items-center cursor-pointer p-3 hover:bg-gray-50 rounded-lg">
                        ...
                        />
                        <span className={!pathname ? 'font-medium text-blue-600' : 'text-gray-700'}>
                          Tất cả sản phẩm
                        </span>
                      </label>
                    </li>
